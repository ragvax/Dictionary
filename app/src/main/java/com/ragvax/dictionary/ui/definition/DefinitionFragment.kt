...
class DefinitionFragment : Fragment(R.layout.fragment_definition) {
    ...
    private fun setupAdapter(meanings: List<WordDefinition.Meaning>) {
        binding.apply {
    ...
    private fun bindOnSuccess(result: WordDefinition) {
        binding.apply {
            requireContext().also {
    ...
    private fun bindOnFailure(errorTitle: String, errorMsg: String) {
        binding.apply {
            ...
            if (errorTitle.isNotBlank()) tvErrorTitle.text = errorTitle
            if (errorMsg.isNotBlank()) tvErrorMessage.text = errorMsg
        }
    }
